...
namespace GradientProjectionMethod
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (First_round = 0; ; First_round++)
                {
                    ...
                    for (int Second_round = 0; Second_round < 1;)
                    {
                        ...
                        for (int i = Count_Points - 1; i > 0; i--)
                            lambda[i] = lambda[i + 1] + Null_lambda * Delta_border_T * Delta_border_T * i;
                        ...
                        for(int Third_round=0; Third_round<1;)
                        {
                            for (int i = 0; i < Count_Points; i++)
                            {
                                ...
                                u1[i] = u0[i] - A * ((Null_lambda * (u0[i]-1) * Delta_border_T)/8 - lambda[i + 1] * Delta_border_T);
                ...
                for (double k = Left_border; k < Right_border; k += 0.001)
                {
                    ...
                    chart1.Series[2].Points.AddXY(k, Math.Pow(1 - u_delta_T[First_round % 2, s], 2) / -16 + Delta_border_T * k * x_delta_T[First_round % 2, s] + u_delta_T[First_round % 2, s] * lambda[s]);// внимательно функция понтрягина
        ...
        private double Quality_Functional(double[] x0, double[] u0, int Count_Points, double Delta_border_T)
        {
            ...
            for (int i = 0; i < Count_Points; i++)
            {
                J += ((1 - u0[i]) * (1 - u0[i]) / 16 - Delta_border_T * i * x0[i])*Delta_border_T;
                    //(Math.Pow(u0[i], 2) - x0[i]) * Delta_border_T;
                    //НЕ ЗАБЫВАЙ ДЕЬТУ ТТТТТТТТ
            }
            return J;
        }    
    }
